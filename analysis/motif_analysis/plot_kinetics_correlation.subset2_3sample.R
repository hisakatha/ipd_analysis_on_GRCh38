...
occ_threshold <- 100
shi2016_mean <- fread("motif_ipd.shi2016_P6C4.summary2.csv")[strand == "+" & region == "Motif" & motif_occ >= occ_threshold, .(position, strand, shi2016 = log2ipd_mean)]

celegans_dir <- "../../../../vs_ce11rel606/jun2018_analysis_high_mapq/motif_analysis/"
ab_mean <- fread(paste0(celegans_dir, motif_dir, "/motif_ipd.ab.c_elegans.summary3.csv"))[strand == "+" & region == "Motif" & motif_occ >= occ_threshold, .(position, strand, ab = log2value_mean)]
cd_mean <- fread(paste0(celegans_dir, motif_dir, "/motif_ipd.cd.c_elegans.summary3.csv"))[strand == "+" & region == "Motif" & motif_occ >= occ_threshold, .(position, strand, cd = log2value_mean)]
...
cairo_pdf("plot_kinetics_correlation.subset2_3sample.pdf", onefile = TRUE, width = 7, height = 7)
